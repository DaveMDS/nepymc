...
def DBG(*args):
    print('CONFIG-GUI:', *args)
    pass


# ### private globals
_browser = None  # EmcBrowser instance
_root_items = []  # ordered list of items. tuple(name, label, weight, icon, cb)
_root_items_dict = {}  # also keep a dict of items:  'name': tuple(...)
...
class StdConfigItemString(StdConfigItemBase):
    ...
    def item_selected(self, url, user_data):
        EmcVKeyboard(title=self._lbl, accept_cb=self._kbd_accept_cb,
                     text=ini.get(self._sec,
                                  self._opt) if not self._pwd else '')
...
class StdConfigItemNumber(StdConfigItemBase):
    ...
    def _dia_text_update(self):
        ...
        self._dia.text_set(
            '<br><br><br><center><bigger>%s</bigger></center>' % val)
...
def init():
    ...
    mainmenu.item_add('config', 100, _('Configuration'),
                      'icon/config', _mainmenu_cb)

    # TODO enable ALL commented items
...
def _general_populate(browser, url):
    standard_item_bool_add('general', 'fullscreen',
                           _('Start in fullscreen'))
    standard_item_bool_add('general', 'hide_mouse',
                           _('Hide mouse cursor when not needed'))

    # TODO enable ALL commneted items
    ...
    standard_item_action_add(_('Clear online images cache'),
                             icon='icon/refresh', cb=_clear_remotes_cache)
...
class ModulesItemClass(EmcItemClass):
    ...
    def icon_end_get(self, url, module):
        return 'icon/check_on' if modules.is_enabled(url) else 'icon/check_off'
...
def _modules_list():
    _browser.page_add('config://modules/', _('Modules'), None,
                      _modules_populate)
...
subs_encs = [
    'UTF-8', 'latin_1', 'iso8859_2', 'iso8859_3', 'iso8859_4', 'iso8859_5',
    'iso8859_6', 'iso8859_7', 'iso8859_8', 'iso8859_9', 'iso8859_10',
    'iso8859_13', 'iso8859_14', 'iso8859_15', 'iso8859_16']


def _subtitles_list():
    _browser.page_add('config://subtitles/', _('Subtitles'),
                      None, _subtitles_populate)


def _subtitles_populate(browser, url):
    standard_item_lang_add('subtitles', 'langs',
        _('Subtitles preferred languages'), multi=True)
    standard_item_string_from_list_add('subtitles', 'encoding',
        _('Subtitles encoding'), subs_encs)
    standard_item_bool_add('subtitles', 'always_try_utf8',
        _('Always try UTF-8 first'))
    standard_item_string_add('subtitles', 'opensubtitles_user',
        _('Opensubtitles.org Username'))
    standard_item_string_add('subtitles', 'opensubtitles_pass',
        _('Opensubtitles.org Password'), pwd=True)


# #############  STORAGE  ######################################################

def _storage_list():
    _browser.page_add('config://storage/', _('Storage devices'),
                      None, _storage_populate)
