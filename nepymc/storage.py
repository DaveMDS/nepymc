...
class EmcDeviceManagerUdev(EmcObject):
    ...
    def udev_device_filter(self, action, udevice, thread=True):
        ...
        if udevice.device_type == 'disk':
            ...
            if udevice.get('ID_CDROM_MEDIA_DVD') == '1' and \
               udevice.get('ID_FS_TYPE') == 'udf':
                self.udev_device_manage(action, udevice, EmcDevType.DVD)
                return

            if udevice.get('ID_CDROM_MEDIA_CD') == '1' and \
               udevice.get('ID_CDROM_MEDIA_TRACK_COUNT_AUDIO') is not None:
    ...
    def udev_device_manage(self, action, udevice, emc_type=None):
        ...
        if action == 'add':
            ...
            if emc_type == EmcDevType.AUDIOCD:
                audio_tracks = int(
                    udevice.get('ID_CDROM_MEDIA_TRACK_COUNT_AUDIO', 0))
            else:
                audio_tracks = 0
