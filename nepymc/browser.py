...
# from nepymc.utils import Singleton
# from nepymc.gui import EmcImage, EmcScrolledEntry, EmcButton, EmcDialog, \
...
# _active_browser = None  # EmcBrowser instance of the currently visible one
...
class EmcItemClass(utils.Singleton):
    ...
    def item_selected(self, url, user_data):
        """ Called when an item is selected by the user in the view """
...
class EmcBrowser(object):
    """
    This is the navigator object, it is used to show various page each containing
    ...
    def _search_style_in_parent(self):
        for p in reversed(self.pages):
            if _memorydb and _memorydb.id_exists(p['url']):
...
class ViewList(object):
    ...
    def item_add(self, item_class, url, user_data, selected=False):
        ...
        gui.text_set('browser.list.total',
                     ngettext('%d item', '%d items', self.items_count) % (
                         self.items_count))
    ...
    def refresh(self):
        ...
        if item:
            self._info_timer_cb(item.item_data_get())
            self._backdrop_timer_cb(item.item_data_get())
    ...
    def selected_url_get(self):
        ...
        if item:
            (item_class, url, user_data) = item.item_data_get()  # 3 #
            return url
    ...
    def _cb_item_realized(self, gl, item):
        ...
        item.signal_emit(
            'icon,show' if item.part_content_get('elm.swallow.icon') \
                else 'icon,hide', 'emc')
    ...
    def _cb_item_hilight(self, gl, item):
        self._clear_timers()
        self._timer1 = ecore.timer_add(0.5, self._info_timer_cb,
                                       item.item_data_get())
        self._timer2 = ecore.timer_add(1.0, self._backdrop_timer_cb,
                                       item.item_data_get())
